...
def handle_send_data(finput):
    global instrument_tokens,instrument_token_2_symbol_mapper,instrument_last_price,kite_obj,input_instruments
    ...
    # instrument_data = get_instrument_tokens(input_instruments,instrument_last_price,20)
    ...
    while True:
        random_name = random.choice(os.listdir('save_data/'))
        print(os.path.join('save_data',random_name))
        data = json.load(open(os.path.join('save_data',random_name),'r'))
        if type(data)==str:
...
class Login(Resource):
    def get(self):
        ...
        login_details = json.dumps(json.load(open('kite_details.json','r')))
        print(login_details)
